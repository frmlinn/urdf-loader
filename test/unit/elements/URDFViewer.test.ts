...
import { flushPromises, waitForLoad } from '../../utils/helpers';
import { getPrivates } from '../../utils/dom-helpers';
...
describe('URDFViewer Web Component', () => {
    ...
    describe('Getters & Setters', () => {
        ...
        it('should correctly map scalar and array values in the jointValues / angles getter', () => {
            const mockRobot = new URDFRobot();
            const jointA = new URDFJoint();
            jointA.name = 'JointA';
            jointA.jointType = 'continuous'; 
            
            const planJoint = new URDFJoint();
    ...
    describe('Public API', () => {
        ...
        describe('updateSize()', () => {
            it('should update camera and WebGLRenderer when size changes', () => {
        ...
        describe('setJointValue() & setJointValues()', () => {
    ...
    // =========================================================================
    // SECTION: Private Methods & Internal Logic
    // =========================================================================
    describe('Private Methods & Logic', () => {
        ...
        describe('_calculateSceneBounds() & _updateShadowBounds()', () => {
            it('should calculate scene bounds and update shadow camera dynamically', () => {
                ...
                viewer.recenter(); // Indirectly invokes _updateShadowBounds
        ...
        describe('_setUp()', () => {
            ...
            it('should fallback to +Z if _setUp is invoked directly with a falsy value', () => {
                ...
                expect(viewer.world.rotation.z).toBeCloseTo(0);
            });
        });

        describe('_setIgnoreLimits()', () => {
            it('should propagate "ignore-limits" flag to all joints if a robot is loaded', () => {
                ...
                viewer.setAttribute('ignore-limits', 'true'); // Automatically calls _setIgnoreLimits via attributeChangedCallback
        ...
        describe('_updateCollisionVisibility()', () => {
            it('should toggle visibility, material, and raycast properties of collisions (showCollision)', () => {
                ...
                viewer.setAttribute('show-collision', ''); // Triggers _updateCollisionVisibility
            ...
            it('should safely skip visibility toggle if a collider mesh has no ancestor flagged as URDFCollider', () => {
                const mockRobot = new URDFRobot();
                const orphanColliderMesh = new THREE.Mesh();
                
                mockRobot.flatColliderMeshes = [orphanColliderMesh];
                viewer.robot = mockRobot;

                expect(() => viewer.setAttribute('show-collision', '')).not.toThrow();
                
                expect(orphanColliderMesh.material).toBe(getPrivates(viewer)._collisionMaterial);
            });
        });
